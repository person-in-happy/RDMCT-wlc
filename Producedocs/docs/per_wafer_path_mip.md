...
- `min c_max + lambda * (full_pm_imbalance + mix_pm_imbalance) + gamma * sum(chamber_idle_slack) + theta * sum(chamber_nonprocess_wait_square) + eta * sum(pair_post_process_wait)`

主目标是最小化“第一片产品 wafer 离开 LP 到最后一片产品 wafer 回到 LP”的端到端总完工时间；`lambda` 为很小的软惩罚权重，用于鼓励 `4x1` 与 `2x2` 产品负载在 CH2/CH3 之间合理分担；`gamma` 用于压缩同一 CH 上相邻加工组之间的可避免空闲；`theta` 用高权重惩罚 CH 内有晶圆但未加工的占腔窗口平方和；`eta` 用于惩罚产品 PW 对在 PM 加工完成后等待 VTR 卸载的时间。`wafer_completion_w` 仅用于将每片晶圆的最终 LP 返回时刻关联到 `c_max`，不再作为独立的逐片完成时刻目标项。`chamber_idle_slack` 包括 `full_batch_idle_*`、`full_to_mix_idle_*` 以及 `2x2` 链内部的 `mix_cycle_to_bridge_idle_*`、`mix_tail_idle_*`。`chamber_nonprocess_wait_square` 覆盖 `4x1` 装入/旋转/卸出、`2x2` head/bridge/tail 和 cleaning 装卸窗口，均按窗口时长分别平方后求和。`2x2` 的 head 完成后立即开始第一个 cycle，bridge 完成后立即开始下一个 cycle；等待只允许发生在前一轮加工结束到 bridge 开始之前，或最后一轮加工结束到 tail 卸片之前。
...
- `ATR`：所有 `LP->AL`、`AL->LLupper`、`LLlower->LP` 动作互斥；对纯 `4x1`、纯 `2x2` 及二者混跑中的每个完整双产品 PW 对，均允许选择双手动作，不能仅因处于混跑模式而把双手变量固定为 `0`。选择双抓 `LP->AL` 后，ATR 可保持携带另一片晶圆并在单槽位 `AL` 内依次完成两片校准，再选择双放至 `LLupper`；双放以前序双抓为条件。未选择双手时，两片晶圆按连续单手动作执行。奇数尾部形成的单产品 PW 对只能采用单手动作。一次 ATR 载片搬运由“取片 + 路径移动 + 放片”组成，其中取片、放片时长均与 VTR 单次搬运时长一致；前一动作终点与后一动作起点不同时，必须插入空载移动。特别地，`LL->LP` 空载回位经过 `AL`，耗时为两倍 `LP->AL` 路径移动时间。
- `AL`：单槽位；每片晶圆放入后立即校准、校准完成后立即由 ATR 取出。双抓场景中另一片或已校准晶圆的等待记在 ATR 持片状态，不记为 AL 等待。
- `LLupper`：双槽位
- `LLlower`：双槽位
