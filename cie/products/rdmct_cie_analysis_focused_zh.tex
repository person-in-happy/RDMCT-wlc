...
偏差分别降低51.91\%、68.89\%、39.70\%和94.05\%。在9个更大构型的729次
分布外实验中，每种方法均获得独立验解有效的可行解。结果明确证明了两阶段模型的
运行价值和结构感知精确搜索在标称范围的计算优势。
\end{abstract}

\begin{highlights}
\item 两阶段MILP统一表示双源混流旋转腔室组合设备调度。
\item 保持最大完工时间的精修显著改善物理等待与腔室节拍。
\item SPBS使平均PDI降低26.19\%，并将可行解获得率提高到100\%。
...
本文采用互补证据检验两项贡献：精确实例验证模型，完整标称矩阵比较算法，控制实验
识别初始解与两阶段机制，析因和分布偏移实验揭示计算难度及规模扩展能力。
...
设$\mathcal W$为产品晶圆，$\mathcal P$为产品组，$\mathcal C$为旋转腔室，
$\mathcal K$为候选配方位置，$\mathcal O$为操作集合，$\mathcal R$为单位容量资源。
$\mathcal K(p)$表示产品组$p$可用的位置，$\mathcal A$为工艺前驱弧，$\mathcal O_r$
为可能使用资源$r$的操作。对腔室$c$，$\mathcal J_c^{\rm DW}$是假片需求集合，
$\mathcal T_c$为物理假片库存。二元变量$x$分配产品组，$u$和$b$启用位置与操作，
$a$分配清洗epoch，$g$把假片需求分配给物理假片，$y$决定共享资源操作次序；
$s_o$、$e_o=s_o+d_o$为操作开始与完成时刻，$\delta_{oo'}$和$\rho^r_{oo'}$
分别表示工艺间隔和端点相关回位间隔，$M$是由排程时域确定的有效上界。核心关系为
\begin{align}
...
完整产品与假片工艺路线，带操作启用条件的成对析取约束统一处理ATR、对准器、装载锁槽位、VTR、
...
清洗epoch满足$\sum_ea_{vce}=b_v$、$a_{vce}\le z_{ce}$和
$\sum_v a_{vce}\le Hz_{ce}$，其中$H$是两次清洗间允许的最大加工
...
本文所指调度死锁是活动搬运动作占有资源并相互等待下一资源所形成的循环等待。
模型使每个活动物料具有从来源到返回端的有限前驱链，并对每对共享单位容量资源的
操作确定唯一时间次序。若路线弧与资源次序形成有向环，把环上具有正间隔的时序
不等式相加将得到$0>0$，因而该组合不可行；终止约束还要求每片产品和已分配假片
完成整条路线。因此，模型的每个可行解都是给定条件下完整且无调度死锁的离线设备
轨迹。
...
不能在此容差之外恶化最大完工时间，也不能改变配方、分配、清洗和资源次序。若
...
\end{equation}
所示的指数移动平均回报基线。实现不含独立学习的critic，故本文准确称其为
A3C启发的并行策略梯度骨干\citep{mnih2016a3c}；创新集中在SPBS、角色重构、
解码和补全。推理时，有限宽束搜索保留多个部分序列\citep{choo2022beam}，
角色次模补全再平衡策略分数、即时割质量、角色覆盖和候选池代表性
\citep{nemhauser1978analysis}。三者存在明确
...
七种条件使用相同且已经独立验解的自动SPBS初始解、时间与内存限制及求解器种子，
因此该表识别割选择与解码差异，不包含SPBS总效应；后者由下一节单独识别。SCIP
...
改善整个求解轨迹。对于有固定计算窗口、需要尽快得到高质量排程的工业场景，
...
该矩阵上的平均观察优势；统计显著的受控增益则来自下述SPBS和第二阶段实验。

\subsection{SPBS显著改善原始搜索}
...
二次目标（关闭线性精修） & \textbf{2469.47} & \textbf{293.27} & 0.134 & 60.18 \\
...
且均通过独立验解。由于全部运行均使用完整搜索窗口，表~\ref{tab:ood-zh-new}采用
PDI和终止gap区分求解轨迹。
...
SPBS--MILP--SCIP求解链则在每次更大分布偏移运行中都交付了验解有效的可行排程。
...
会保留大量可避免等待和节拍波动。第二，应用可行初始解给出本文测得的最大受控计算改善，SPBS
应被视为求解架构而不是实现细节。第三，学习表示必须对应MIP物理角色。束搜索效应
的观察符号反转与其和结构感知表示之间的耦合相一致：当候选表示已经对齐离散设备
选择与连续时刻后，束解码才改善观察均值；该组件比较本身并未检验统计交互。

本文显式分离的状态、资源和时序层也为在线到达、随机加工及模块故障追索提供直接
扩展基础。有限批次证据已经确立核心结论：对双源混流设备，两阶段模型产生运行质量
显著更优的排程，\method{}在比较方法中取得最低观察标称平均PDI和时间。
...
属于重构特征所表示的同类离散--连续耦合。9个更大分布偏移构型的729次运行也
全部产生独立验解有效的可行解，确立了规模外可行排程交付能力。

因此，本文结论是明确的：所提两阶段MILP能够产生运行质量更优的排程，结构感知学习
增强分支割在本次比较中取得最优观察标称平均anytime性能。二者共同解决了现有单源或固定序列
